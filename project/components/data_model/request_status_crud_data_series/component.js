if (typeof component_classes['data_model_request_status_crud_data_series'] === "undefined") {
	class data_model_request_status_crud_data_series extends DivbloxDomBaseComponent {
		...
		deleteSelected() {
			dxRequestInternal(getComponentControllerPath(this),
				...
				function(data_obj) {
					...
					this.current_page = 1;
					this.loadPage();
					pageEventTriggered("request_status_selection_deleted",{});
				}.bind(this),
				function(data_obj) {
		...
		loadPage() {
			let uid = this.getUid();
			let search_text = getComponentElementById(this,"DataTableSearchInput").val();
			getComponentElementById(this,"DataTableBody").html('<tr id="'+uid+'_DataTableLoading"><td colspan="2"' +
		...
		on_item_clicked(id) {
			setGlobalConstrainById("RequestStatus",id);
			pageEventTriggered("request_status_clicked",{id:id});
		}
	}
	component_classes['data_model_request_status_crud_data_series'] = data_model_request_status_crud_data_series;
}