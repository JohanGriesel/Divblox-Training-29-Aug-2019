if (typeof component_classes['data_model_open_stock_request_feed'] === "undefined") {
	class data_model_open_stock_request_feed extends DivbloxDomBaseComponent {
		constructor(inputs,supports_native,requires_native) {
			...
			this.total_items = 0;
			this.current_sort_column = ["RequestText",true];
		}
		reset(inputs) {
			this.current_page_array = [];
		...
		addRow(row_data_obj) {
			...
			let html = '<a href="#" id="'+this.uid+'_row_item_'+row_id+'" class="list-group-item list-group-item-action' +
				...
				'            <div class="d-flex w-100 justify-content-between">\n' +
					...
					'                <small>'+row_data_obj['Account']+'</small>\n' +
			'            </div>\n' +
			'            <p class="mb-1">'+row_data_obj['RequestText']+'</p>\n' +
				'            <small>'+row_data_obj['CreatedDateTime']+'</small>\n' +
			'</a>';
			getComponentElementById(this,"DataList").append(html);
		}
		// Data table functions to implement
		on_item_clicked(id) {
			setGlobalConstrainById("StockPhotoRequest",id);
			pageEventTriggered("stock_photo_request_clicked",{id:id});
		}
	}
	component_classes['data_model_open_stock_request_feed'] = data_model_open_stock_request_feed;
}