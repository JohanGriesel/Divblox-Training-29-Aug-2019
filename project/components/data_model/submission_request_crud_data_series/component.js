if (typeof component_classes['data_model_submission_request_crud_data_series'] === "undefined") {
	class data_model_submission_request_crud_data_series extends DivbloxDomBaseComponent {
		constructor(inputs,supports_native,requires_native) {
			...
			*/
            ...
            "PhotoFilePath":"Photo File Path",
            "Account":"Account",
            "StockPhotoRequest":"Stock Photo Request",
		...
		deleteSelected() {
			dxRequestInternal(getComponentControllerPath(this),
				...
				function(data_obj) {
					...
					this.current_page = 1;
					this.loadPage();
					pageEventTriggered("photo_submission_selection_deleted",{});
				}.bind(this),
				function(data_obj) {
		...
		loadPage() {
			let uid = this.getUid();
			let search_text = getComponentElementById(this,"DataTableSearchInput").val();
			getComponentElementById(this,"DataTableBody").html('<tr id="'+uid+'_DataTableLoading"><td colspan="8"' +
		...
		on_item_clicked(id) {
			setGlobalConstrainById("PhotoSubmission",id);
			pageEventTriggered("photo_submission_clicked",{id:id});
		}
	}
	component_classes['data_model_submission_request_crud_data_series'] = data_model_submission_request_crud_data_series;
}