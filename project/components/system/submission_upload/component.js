if (typeof component_classes['system_submission_upload'] === "undefined") {
	class system_submission_upload extends DivbloxDomBaseComponent {
		...
		initFileUploader() {
			...
			$('#'+uid+'_file_uploader').fileuploader({
				...
				upload: {
					...
					data: {f:"handleFilePost",AuthenticationToken:getValueFromAppState('dxAuthenticationToken'),request_id:getGlobalConstrainById("StockPhotoRequest")},
					type: 'POST',
					...
					onError: function(item) {
                        dxLog("Returned: "+JSON.stringify(item));
						var progressBar = item.html.find('.progress-bar2');
						if(progressBar.length) {
							progressBar.find('span').html(0 + "%");
							progressBar.find('.fileuploader-progressbar .bar').width(0 + "%");
	...
	component_classes['system_submission_upload'] = system_submission_upload;
}